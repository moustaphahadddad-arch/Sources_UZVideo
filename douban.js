  ...
  try {
    ...
    try {
      ...
      const requestData = {
        page_params: {
          ...
          ad_exp_ids: "100000",
          ams_cookies:
            "lv_play_index=26; o_minduid=CpGFdExDeM8uP-XHCyma_0PzurMADpcf; appuser=83C1297D3AE9DEFF",
      ...
      if (
        tencentBannerResponse.statusCode === 200 &&
        tencentBannerResponse.body
      ) {
        let tencentBannerBodyStr =
          typeof tencentBannerResponse.body === "string"
            ? tencentBannerResponse.body
            : String(tencentBannerResponse.body);
        const tencentBannerData = JSON.parse(tencentBannerBodyStr);

        // 解析腾讯响应数据，提取轮播图卡片
        if (
          tencentBannerData.data &&
          tencentBannerData.data.CardList &&
          Array.isArray(tencentBannerData.data.CardList)
        ) {
          for (const cardList of tencentBannerData.data.CardList) {
            // 只处理轮播图类型的卡片
            if (
              cardList.type === "pc_carousel" &&
              cardList.children_list &&
              cardList.children_list.list &&
              ...
              for (const card of cards) {
                ...
                if (card.params && card.params.pic_ori_2880x900) {
                  // 处理背景图片URL
                  let backgroundImage =
                    card.params.pic_ori_2880x900 || card.params.image_url || "";

                  // 如果图片URL是外部URL，需要通过代理
                  if (
                    backgroundImage &&
                    baseURL &&
                    !backgroundImage.includes(baseURL) &&
                  ...
                  banner.push({
                    ...
                    subtitle:
                      card.params.stitle_pc ||
                      card.params.material_video_subtitle ||
                      "",
    ...
    try {
      ...
      if (tvListResponse.statusCode === 200 && tvListResponse.body) {
        let tvListBodyStr =
          typeof tvListResponse.body === "string"
            ? tvListResponse.body
            : String(tvListResponse.body);
        ...
        if (tvListData.items && Array.isArray(tvListData.items)) {
          ...
          list = tvListData.items.map((item) => {
            ...
            return {
              ...
              vod_year: vod_year,
              vod_douban_score: item.rating?.value
                ? item.rating.value.toString()
                : "",
...
async function category(params, context) {
  try {
    ...
    if (categoryId === "follow") {
      try {
        const categoryData = await OmniBox.getSourceCategoryData(
          "follow",
          page,
          limit,
        );
        const list = (categoryData.list || []).map((item) => {
          let vod_pic = item.vod_pic || item.VodPic || "";
          return {
        ...
        const pagecount =
          categoryData.pageCount ?? (total > 0 ? Math.ceil(total / limit) : 0);
        return {
          page: page,
          pagecount: pagecount,
          total: total,
        ...
        await OmniBox.log("warn", `获取追剧列表失败: ${followErr.message}`);
        return {
          page: page,
          pagecount: 0,
          total: 0,
    ...
    if (categoryId === "movie") {
      ...
      const type =
        filters.type || (categoryId === "tv" ? "tv_domestic" : "show");
